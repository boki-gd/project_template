struct Thread_param{
   Platform_data* memory;
   D3D* dx;
   Asset_request* request;
   // Memory_arena* assets_arena;
	Dx_assets* dx_assets;
};
...
FUNCTION_TYPE_WORK_CALLBACK(threaded_request_tex_from_file)
{
   ...
   Asset_request* request = thread_param->request;
   // Memory_arena* assets_arena = thread_param->assets_arena;
   // Dx11_texture_view*** textures_list = *thread_param->textures_list;
	Dx_assets* dx_assets = thread_param->dx_assets;
   // LIST(Dx11_texture_view*, textures_list) = {
   //    thread_param->textures_list[0], 
   //    thread_param->textures_list[1],
   ...
   // stbi_convert_wchar_to_utf8(temp_buffer, request->filename.length, (wchar_t*)request->filename.text);
	...
	texture2d->Release();
	free(tex_surface.data);
	
	// this is done at the end due to multithreading
   tex_info->texture_uid = new_tex_i;
   *request->p_uid = new_tex_info_i;

	// char buffer[256];
	// wsprintf(buffer, "Thread %u (index %u)\n", GetCurrentThreadId(), thread->thread_index);
...
internal void
process_asset_requests(Platform_data* memory, 
	Memory_arena* assets_arena,
	D3D* dx,
	u32 ie_formats_sizes[],
	DXGI_FORMAT ie_formats_list[],
	D3D11_PRIMITIVE_TOPOLOGY topologies_list [TOPOLOGIES_COUNT],
	D3D_constant_buffer renderer_variables [],
	HWND window,
	Dx_assets* dx_assets,
	Sound_sample sounds_list [],
	Win32_thread_info thread_infos[]
)
{thread_infos;
	Memory_arena* temp_arena = memory->temp_arena;
	Memory_arena* permanent_arena = memory->permanent_arena;
	...
	FOREACH(Asset_request, request, memory->asset_requests)
	{
		switch(request->type){
            ...
            *permanent_request = *request;
            ...
            copy_mem(request->filename.text, permanent_request->filename.text, request->filename.length);

            Thread_param* thread_param = ARENA_PUSH_STRUCT(assets_arena, Thread_param);
            thread_param->memory = memory;
            thread_param->dx = dx;
            thread_param->request = permanent_request;
            // thread_param->assets_arena = assets_arena;
            thread_param->dx_assets = dx_assets;
			...
			case ASSET_REQUEST_ALLOC_MEMORY:
			{	
				*request->alloc_memory.data = VirtualAlloc(0, request->alloc_memory.size, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
			}break;
			
			//TODO: make a helper program that converts any binary file into an array of bytes represented as a string like this.
			/*
			...
			*/
			case ASSET_REQUEST_VS_FROM_FILE:{
				...
				u32 current_index = LIST_SIZE(dx_assets->vertex_shaders_list);
				ASSERT(current_index < 0xffff);
				*request->p_uid = (u16)current_index;
				Vertex_shader* vs; PUSH_BACK(dx_assets->vertex_shaders_list, assets_arena, vs);
			...
			case ASSET_REQUEST_PS_FROM_FILE:{
				u32 current_index = LIST_SIZE(dx_assets->pixel_shaders_list);
				ASSERT(current_index < 0xffff);
				*request->p_uid = (u16)current_index;
				// COMPILING PS
				...
				Pixel_shader* ps; PUSH_BACK(dx_assets->pixel_shaders_list, assets_arena, ps);
			...
			case ASSET_REQUEST_MESH_FROM_FILE:
			{
				...
				u32 current_index = LIST_SIZE(dx_assets->meshes_list);
				ASSERT(current_index < 0xffff);
				*request->p_uid = (u16)current_index;
				
				Dx_mesh* current_mesh; PUSH_BACK(dx_assets->meshes_list, assets_arena, current_mesh);
			...
			case ASSET_REQUEST_CREATE_BLEND_STATE:{
				u32 current_index = LIST_SIZE(dx_assets->blend_states_list);
				ASSERT(current_index < 0xffff);
				*request->p_uid = (u16)current_index;

				Dx11_blend_state** blend_state; PUSH_BACK(dx_assets->blend_states_list, assets_arena, blend_state);
			...
			case ASSET_REQUEST_CREATE_DEPTH_STENCIL:{
				u32 current_index = LIST_SIZE(dx_assets->depth_stencils_list);
				ASSERT(current_index < 0xffff);
				*request->p_uid = (u16)current_index;

				Depth_stencil* depth_stencil; PUSH_BACK(dx_assets->depth_stencils_list, assets_arena, depth_stencil);
			...
			case ASSET_REQUEST_FONT_FROM_FILE:
			{
				...
				// u16 atlas_texview_uid = (u16)InterlockedCompareExchange(&dx_assets->tex_count, original_tex_count+1, original_tex_count);
				u16 atlas_texview_uid = find_first_available_i(dx_assets->textures_list_b, ARRAYCOUNT(dx_assets->textures_list_b));
				...
				*request->font_uid = (u16)LIST_SIZE(memory->fonts_list);
				...
				UNTIL(i, CHARS_COUNT){
					...
					if(rects[i].was_packed){
						...
						new_font->texinfo_uids[i] = find_first_available_i(ARRAY_ARRAYCOUNT(memory->tex_infos_b));
						Tex_info* charinfo = &memory->tex_infos[new_font->texinfo_uids[i]];
				...
				new_font->atlas_texinfo_uid = find_first_available_i(ARRAY_ARRAYCOUNT(memory->tex_infos_b));
				Tex_info* atlas_tex_info = &memory->tex_infos[new_font->atlas_texinfo_uid];
				...
				Dx11_texture_view** atlas_texture = &dx_assets->textures_list[atlas_texview_uid];
			...
			case ASSET_REQUEST_TEX_FROM_SURFACE:{
				u32 current_index = find_first_available_i(ARRAY_ARRAYCOUNT(memory->tex_infos_b));
				ASSERT(current_index < 0xffff);
				*request->p_uid = (u16)current_index;

				Tex_info* tex_info = &memory->tex_infos[*request->p_uid];
				...
				tex_info->texture_uid = find_first_available_i(ARRAY_ARRAYCOUNT(dx_assets->textures_list_b));
				Dx11_texture_view** texture_view = &dx_assets->textures_list[tex_info->texture_uid];
			...
			case ASSET_REQUEST_CREATE_DYNAMIC_TEXTURE:{
				...
				else if(request->dynamic_tex_sizes.x && request->dynamic_tex_sizes.y && !request->dynamic_tex_sizes.z)
				{
					...
					HRESULT hr = dx->device->CreateTexture2D(&tex2d_desc, 0, &texture2d);
					ASSERTHR(hr);
					
					*request->p_uid = find_first_available_i(ARRAY_ARRAYCOUNT(dx_assets->textures_list_b));
					Dx11_texture_view** texture_view = &dx_assets->textures_list[*request->p_uid];
					dx11_create_texture_view(dx, texture2d, texture_view);
					texture2d->Release();
				...
				else if(request->dynamic_tex_sizes.x && request->dynamic_tex_sizes.y && request->dynamic_tex_sizes.z)
				{
					...
					HRESULT hr = dx->device->CreateTexture3D(&tex3d_desc, 0, &texture3d);
					ASSERTHR(hr);
			...
			{
				*request->p_uid = (u16)LIST_SIZE(dx_assets->render_targets_list);
				Render_target* new_render_target;
				PUSH_BACK(dx_assets->render_targets_list, assets_arena, new_render_target);
				
				u16 new_tex_i = find_first_available_i(ARRAY_ARRAYCOUNT(dx_assets->textures_list_b));
				new_render_target->texture_view = &dx_assets->textures_list[new_tex_i];
			}break;
			...
			case ASSET_REQUEST_CREATE_DYNAMIC_MESH:{
				...
				*request->p_uid = (u16)current_index;
				...
				PUSH_BACK(dx_assets->meshes_list, assets_arena, current_mesh);
